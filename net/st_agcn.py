...
class STA_GCN(nn.Module):
    ...
    def forward(self, x):
        ...
        ## reduce the vertex dimension
        PA_embedding = F.avg_pool2d(PA_embedding, (PA_embedding.size(2), 1)).squeeze(2)

        output_embedding = PA_embedding

        ## Dense layer to meet T5 input
