...
def align(query_embs,input_embs) -> (np.ndarray,np.ndarray):
    ...
    def find_min_distance_with_standard(emb,query_embs):
        ...
        if len(emb) <= len(query_embs): ## There is no need to pad the emb, it is guranteed to be played at the first frame. (if we think about the sliding window algo)
            return 0
        
        for i in range(len(emb)-len(query_embs)): ## * use sliding window
            query_embs = emb[i:i+len(query_embs)]   ## * compare in which window
            min_dist, _, _, _ = dtw(query_embs, input_embs, dist=dist_fn) ## * the dtw yields
