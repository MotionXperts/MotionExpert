...
def eval(args,eval_dataloader, model, tokenizer,epoch,summary_writer,sanity_check=False,store=None,name_list = None,logger=None):       
    ...
    att_node_results = {}
    att_A_results = {}  
    prompt =  "Motion Instruction : " if args.finetune == True else "Motion Description : "
    with torch.no_grad():
        if dist.get_rank() == 0:
            eval_dataloader = tqdm(eval_dataloader, total=len(eval_dataloader), desc='Evaluating')
        for index,batch in enumerate(eval_dataloader):
            video_names = batch['video_name']
            src_batch = batch['keypoints'].to(model.device)
            ...
            input = {   "input_embeds": src_batch.contiguous(),
                        "decoder_input_ids": decoder_input_ids,
                        "name": video_names,
                        "tokenizer": tokenizer,
                        "fine_tune": args.finetune,
                        "result_dir": os.path.join(args.out_dir,'jsons'),
                        "epoch": epoch }
            with torch.cuda.amp.autocast():
                generated_ids , att_node , att_A = model.module.generate(**input)
                loss = model.module(keypoints=src_batch.contiguous(), 
